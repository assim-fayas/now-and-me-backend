...
const adminlogin = async (req, res) => {
  ...
  const token = jwt.sign(
    ...
    process.env._JWT_ADMIN_SECERETKEY,
    { expiresIn: 3600 }
  );
  console.log(token);
...
const adminDashboard = async (req, res) => {
  try {
    ...
    const totalAdminRevenue =
      ((await chatrevenue) + (await videorevenue)) * 0.3;
    console.log(totalAdminRevenue);

    // chat count
    const countOfChat = await Appointment.find({
      $and: [{ bookingType: "chat", status: "consulted" }],
    }).count();
    console.log(countOfChat);
    //video chat count
    const countOfVideoChat = await Appointment.find({
      $and: [{ bookingType: "video", status: "consulted" }],
    }).count();
    console.log(countOfVideoChat);
    //active appoinments
    const activeAppoinment = await Appointment.find({
      $and: [{ AppoinmentStatus: "active" }],
    }).count();
    return res.status(200).json({
      chat: countOfChat,
      video: countOfVideoChat,
...
const adminPieChartData = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send([{ chat: countOfChat }, { video: countOfVideoChat }]);
  } catch (error) {
    console.log(error);
  }
};
...
const topPerformers = async (req, res) => {
  try {
        ...
        $addFields: {
          earningsByBookingType: {
            $concatArrays: [
              {
                $filter: {
                  input: "$earningsByBookingType",
                  as: "item",
                  cond: { $eq: ["$$item.bookingType", "chat"] },
                },
              },
              {
                $filter: {
                  input: "$earningsByBookingType",
                  as: "item",
                  cond: { $eq: ["$$item.bookingType", "video"] },
                },
              },
              {
                $cond: {
                  ...
                  then: [],
                  else: [{ bookingType: "chat", totalEarnings: 0 }],
                },
              },
              {
                $cond: {
                  ...
                  then: [],
                  else: [{ bookingType: "video", totalEarnings: 0 }],
                },
              },
            ],
          },
    ...
    console.log(earningsByExpertAndBookingType);
    return res.status(200).json({ earningsByExpertAndBookingType });
  } catch (error) {
    console.error("Error:", error);
    res
      .status(500)
      .json({ error: "An error occurred while fetching top performers." });
  }
};
...
const blockPost = async (req, res) => {
  try {
    ...
    if (post.block == true) {
      const blockPost = await Post.updateOne(
        { _id: postId },
        { $set: { block: false } }
      );
      if (blockPost) {
    ...
    if (blockPost) {
      ...
      return res
        .status(200)
        .send({ message: "Post Blocked and Email send successfully" });
    }
  } catch (error) {
    console.log(error);
  }
