...
const ActiveChats = async (req, res) => {
  try {
    ...
    const activeChats = await Appointment.find({
      user: userId,
      bookingType: "chat",
      AppoinmentStatus: "active",
...
const previousChats = async (req, res) => {
  try {
    ...
    const findAllchatWitUser = await Appointment.find({
      user: user,
      AppoinmentStatus: "expired",
      bookingType: "chat",
    }).populate("expert", "name");
    ...
    for (const obj of findAllchatWitUser) {
      const expertId = obj.expert?._id.toString();
    ...
    console.log(uniqueChats, "uniquee");
